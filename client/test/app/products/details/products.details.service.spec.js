...
describe('ProductsDetailsService', function(){
    var scope,
        sampleSvcObj,
        $q,
        $httpBackend,
    ...
    beforeEach(inject(function($rootScope, _$httpBackend_, ProductsDetailsService, _$q_, ADAMS_URL_SPACE, $log){
        scope = $rootScope;
        $httpBackend = _$httpBackend_;
        sampleSvcObj = ProductsDetailsService;
        $q = _$q_;
    ...
    it('should getProductDetails  Info', function(){
        var gtin = '100',
            url = urlSpace.urls.local.productDetails.replace('{gtin}', gtin);
        var responseData = {
            data: {
                data: [{}]
            }
        ...
        sampleSvcObj.getProductDetails (gtin).then(function(response) {
            expect(response).toEqual(undefined);
        });
        $httpBackend.flush();
    });

    it('should throw error getProductDetails Info', function(){
        var gtin = '100',
            url = urlSpace.urls.local.productDetails.replace('{gtin}', gtin);
    ...
    it('should getProductImages  ', function(){
        var gtin = '100',
            url = urlSpace.urls.local.multimedia.replace('{gtin}', gtin);
        var responseData = {
            data: {
                data: []
            }
        ...
        $httpBackend.expectGET(url).respond(responseData);
        sampleSvcObj.getProductImages(gtin).then(function(response) {
            expect(response).toEqual(responseData.data);
        });
        $httpBackend.flush();
    });

    it('should throw error getProductImages  ', function(){
        var gtin = '100',
            url = urlSpace.urls.local.multimedia.replace('{gtin}', gtin);
        $httpBackend.expectGET(url).respond(400, {});
        sampleSvcObj.getProductImages(gtin).then(function(response) {
