...
describe('ProductsSearchService', function(){
    var scope,
        sampleSvcObj,
        $q,
        $httpBackend,
    ...
    beforeEach(inject(function($rootScope, _$httpBackend_, ProductsSearchService, _$q_, ADAMS_URL_SPACE, $log){
        scope = $rootScope;
        $httpBackend = _$httpBackend_;
        sampleSvcObj = ProductsSearchService;
        $q = _$q_;
        $log = $log;
    ...
    it('should get Product Info', function(){
        var limit = 25,
            page = 1,
            search = '',
            sort = '',
            url = urlSpace.urls.local.products + '?limit=' + limit + '&page=' + page  + '&search=' + JSON.stringify(search) + '&sorts=' + sort;

        spyOn(promise, 'abort').and.callThrough();
        $httpBackend.expectGET(url).respond(productSearchData);
        sampleSvcObj.getAllProductsSearchDetails(limit, page, search, sort).then(function(data) {
            expect(data).toEqual(productSearchData);
        });
        $httpBackend.flush();
    });

    it('should abort get Product Info promise', function(){
        var limit = 25,
            page = 1,
            search = '',
            sort = '',
        ...
        $httpBackend.expectGET(url).respond(productSearchData);
        sampleSvcObj.getAllProductsSearchDetails(limit, page, search, sort).abort();
    });

    it('should return empty product Info on error ', function(){
        var limit = 25,
            page = 1,
            search = '',
            sort = '',
