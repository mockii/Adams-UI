...
describe('VendorDetailsService', function(){
    ...
    beforeEach(inject(function($rootScope, _$httpBackend_, VendorDetailsService, _$q_, ADAMS_URL_SPACE){
        scope = $rootScope;
        $httpBackend = _$httpBackend_;
        sampleSvcObj = VendorDetailsService;
        $q = _$q_;
    ...
    it('should get Vendor Detail Info', function(){
        var limit = 25,
            ...
            sourceSystemId = '1',
            vendorNumber = '2',
            url = urlSpace.urls.local.vendorDetails + '?sourceSystemId=' + sourceSystemId+ '&vendorNumber=' + vendorNumber;
        vendorSearchData = JSON.parse(vendorSearchData);
        $httpBackend.expectGET(url).respond(vendorSearchData);
        sampleSvcObj.getVendorDetails(vendorNumber, sourceSystemId).then(function(response) {
            expect(response).toEqual(vendorSearchData.data[0]);
        });
        $httpBackend.flush();
    });

    it('should throw error Vendor Detail Info', function(){
        var limit = 25,
            page = 1,
            sourceSystemId = '1',
            vendorNumber = '2',
    ...
    it('should updateVendorComment ', function(){
        var sourceSystemId = '1',
            vendorNumber = '2',
            url = urlSpace.urls.local.vendorDetails + '?sourceSystemId=' + sourceSystemId+ '&vendorNumber=' + vendorNumber;
        // vendorSearchData = JSON.parse(vendorSearchData);
        var response = {
        ...
        $httpBackend.expectPATCH(url).respond(vendorSearchData);
        sampleSvcObj.updateVendorComment(vendorNumber, sourceSystemId, '').then(function(response) {
            expect(response).toEqual(vendorSearchData.data[0]);
        });
        $httpBackend.flush();
    });

    it('should throw error updateVendorComment ', function(){
        var sourceSystemId = '1',
            vendorNumber = '2',
            url = urlSpace.urls.local.vendorDetails + '?sourceSystemId=' + sourceSystemId+ '&vendorNumber=' + vendorNumber;
